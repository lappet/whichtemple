#!/usr/bin/env python3
"""Fetch all temples in Odisha from Wikidata, with the same columns as
hindu_temples.csv.

Selector: temples (P31 any subclass of Q44539) located, directly or through
the administrative hierarchy (P131*), in Odisha (Q22048), that have an
image (P18).
...
Usage:
  python3 wikidata_odisha_temples.py --out ../public/odisha_temples.csv
"""
...
DEFAULT_OUT = Path(__file__).resolve().parent.parent / "public" / "odisha_temples.csv"

SELECTOR = """
  ?t wdt:P31/wdt:P279* wd:Q44539 .    # temple (incl. subclasses)
  ?t wdt:P131* wd:Q22048 .            # in Odisha, via the admin hierarchy
...
def main() -> None:
    ...
    print("Fetching Odisha temple entity set...", file=sys.stderr)
    ...
    for r in rows:
        if r and r[0]:
            temples[wht.qid(r[0])] = {
                c: "" for c in ["state_name", "lat", "lon"] + attr_cols
            }
